...
var createParticles = function () {
    ...
    geometry.setAttribute('index', new THREE.Uint16BufferAttribute(indices, 1).onUpload( disposeArray ));
    geometry.setAttribute('normal', new THREE.Float32BufferAttribute(new Float32Array(triangles * 3), 3).onUpload( disposeArray ));
    geometry.setAttribute('lng', new THREE.Float32BufferAttribute(lng_values, 1).onUpload( disposeArray ));

    geometry.groups = [];
    var offsets = triangles / chunkSize;

    for (var i = 0; i < offsets; i++) {
        geometry.addGroup(i * chunkSize * 3, Math.min(triangles - (i * chunkSize), chunkSize) * 3, i * chunkSize * 3);
    }
